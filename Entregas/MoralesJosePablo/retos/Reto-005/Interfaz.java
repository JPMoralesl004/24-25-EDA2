...
public class Interfaz {
    ...
    private IndiceOrdenado indiceOrdenado;

    public Interfaz (HojaDeCalculo hoja) {
        this.viewport = new Viewport(hoja, 15, 10);
        this.scanner = new Scanner(System.in);
        this.indiceOrdenado = new IndiceOrdenado(hoja.getNumeroDeFilas());
    }
    ...
    private void mostrarHoja() {
        ...
        for (int i = 0; i < viewport.getFilasViewport(); i++) {
            int filaReal = indiceOrdenado.getIndiceReal(viewport.getFilaInicio() + i);
            System.out.printf("%-5d|", filaReal + 1);

            for (int j = 0; j < viewport.getColumnasViewport(); j++) {
                String celda = viewport.getViewport().getCelda(filaReal, viewport.getColumnaInicio() + j).getContenido();
                celda = celda.length() > 5 ? celda.substring(0, 5) : String.format("%-5s", celda);

                if (filaReal == viewport.getFilaCursorGlobal() && j == viewport.getColumnaCursorGlobal() - viewport.getColumnaInicio()) {
    ...
    private boolean procesarComando(char comando) {
        switch (comando) {
            case 'W': viewport.moverCursor(-1, 0); break;
            case 'A': viewport.moverCursor(0, -1); break;
            case 'S': viewport.moverCursor(1, 0); break;
            case 'D': viewport.moverCursor(0, 1); break;
            case 'E': editarCeldaActual(); break;
            case 'O': ordenarColumnaActual(); break;
            case 'Q': return false;
            default:
    ...
    private void ordenarColumnaActual() {
        ...
        indiceOrdenado.ordenarPorColumna(viewport.getViewport(), columna, opcion);
    }
}